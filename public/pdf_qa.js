...
function output(val, person, priority) {
    ...
    //テキストを表示
    if(person == "Q"){
        let li = viewQ(sendTime,priority,val,"Q"+ Q_num);
        dataQA[Q_num]={Q:li.id, A:[],state:"add",sendtime:sendTime,priority:priority,content:val}
...
//質問の画面への出力部分
function viewQ(sendTime,priority,val,id){
    ...
    li.style="display:list-item block;"
    ul.appendChild(li);
    div.innerHTML = "[" + id + "]    Send: "+ sendTime + "    /" + priority + "<br>  " + val;
    li.appendChild(div);
    return li
    }
